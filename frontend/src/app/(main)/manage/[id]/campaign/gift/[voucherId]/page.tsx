...
export default function PromotionGiftCreatePage() {
    ...
    const fetchPromotionDetail = async () => {
      ...
      try {
        const res = await axiosInstance.get(
          `${apiLinks.promote.createPromotionGift}/${projectId}/${promotionId}`
        )        
        ...
        if (data) {
          dispatch(resetPromotionGift())
          dispatch(setPromotionGift(data))
          // dispatch(updatePromotionGiftField({
          //   name: data.name || "",
          //   startTime: data.startTime || "",
          //   endTime: data.endTime || "",
          // }))

          // // Cập nhật danh sách sản phẩm (product)
          ...
          // // Cập nhật productApply
          // if (data.productApply && Array.isArray(data.productApply)) {
          //   const uniqueProductApply = data.productApply.map((item: any) => ({
          //     promoteTarget: item.promoteTarget || 0,
          //     productGift: item.productGift || [],
          //   }))
          //   dispatch(updatePromotionGiftField({ productApply: uniqueProductApply }))

          //   data.productApply.forEach((item: any, index: number) => {
          //     if (item.productGift && Array.isArray(item.productGift)) {
          //       item.productGift.forEach((gift: Product) => {
          //         dispatch(addProductToProductGiftApply({ index, product: gift }))
          //       })
          //     }
          //   })
          // }
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (projectId) {
        if (promotionId) {
          await promotionService.updatePromotionGift(postPromotionGift, projectId, promotionId)
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle={promotionId ? "Chỉnh sửa quà tặng khi mua hàng" : "Tạo quà tặng khi mua hàng"} />        <ComponentCard title="Thông tin cơ bản">
          <div className="space-y-1 grid grid-cols-1 gap-6 sm:grid-cols-2">
            ...
            <div className="col-span-1">
              <DatePicker
                ...
                defaultDate={promotionGift.startTime ? new Date(promotionGift.startTime) : undefined}
            ...
            <div className="col-span-1">
              <DatePicker
                ...
                minDate={minEndDate}
                defaultDate={promotionGift.endTime ? new Date(promotionGift.endTime) : undefined}
              ...
              <ComponentCard key={index} title={`Bậc ${index + 1}`}>
                <div className="space-y-4">
                  ...
                  {productGiftApply.productGift.length > 0 ? (
                    ...
                    <div className="text-center py-8">
                      <p className="text-gray-500 dark:text-gray-400">Chưa có sản phẩm quà tặng nào được thêm</p>
                    </div>
                  )}
                </div>
              </ComponentCard>

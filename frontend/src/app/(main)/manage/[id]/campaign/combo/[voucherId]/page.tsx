...
interface PostProductApply {
  productId: string;
  promoteType: "percent" | "fixed";
  promotePricing: number;
}

interface PostPromotion {
  name: string;
  startTime: string;
  endTime: string;
  promotionId: string;
...
export default function DiscountCreatePage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (projectId) {
        if (promotionId) {
          // Cập nhật khuyến mãi
          await promotionService.updatePromotionDiscount(postPromotion, projectId, promotionId);
          toast.success("Cập nhật khuyến mãi thành công");
          dispatch(fetchVouchers(projectId));
        } else {
          // Tạo mới khuyến mãi
          await promotionService.createPromotionDiscount(postPromotion, projectId);
  ...
  return (
    <div>
      <PageBreadcrumb
        pageTitle={promotionId ? "Chỉnh sửa ưu đãi combo tiết kiệm" : "Tạo ưu đãi combo tiết kiệm"}
      />
      <ComponentCard title="Thông tin cơ bản">
        <div className="space-y-1 grid grid-cols-1 gap-6 sm:grid-cols-2">
          <div className="col-span-2">
            <Label className="text-sm text-gray-500">Tên khuyến mãi</Label>
            <Input
              type="text"

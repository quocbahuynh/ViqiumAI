...
export default function PromotionBulkCreatePage() {
  ...
  const handleSubmit = async () => {
    ...
    if (promotionBulk.productApply[0].promoteTarget <= 0) {
      toast.error("Số lượng mua của bậc 1 phải lớn hơn 0")
      return
    }
    if (promotionBulk.productApply.some((item) => item.promoteTarget <= 0)) {
      toast.error("Số lượng mua phải lớn hơn 0")
      return
    }
    if (promotionBulk.productApply.some((item) => item.promoteType !== "percent")) {
      toast.error("Chỉ hỗ trợ loại giảm giá theo phần trăm")
      return
    }
    ...
    try {
      if (projectId) {
        const result = await promotionService.createPromotionBulk(postPromotionBulk, projectId)
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle="Tạo mua sỉ giá hời
" />
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-items-  gap-4">
                    ...
                    <div className="flex-col">
                      ...
                      />
                    </div>                    hoặc nhiều sản phẩm hơn, được giảm

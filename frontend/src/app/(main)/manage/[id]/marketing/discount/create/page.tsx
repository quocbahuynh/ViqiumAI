...
export default function DiscountCreatePage() {
  ...
  const handleSubmit = async () => {
    ...
    if (promotion.productApply[0].promoteType === "percent" && percentValue <= 0) {
      toast.error("Vui lòng nhập phần trăm giảm giá lớn hơn 0")
      return
    }
    if (promotion.startTime >= promotion.endTime) {
      toast.error("Thời gian kết thúc phải sau thời gian bắt đầu")
    ...
    try {
      if (projectId) {
        const result = await promotionService.createPromotionDiscount(postPromotion, projectId)
  ...
  return (
    <div>
      <PageBreadcrumb pageTitle="Tạo giảm giá sản phẩm" />
      ...
      <ComponentCard className="mt-6">
        <div className="space-y-4">
          ...
          {promoteType === "percent" && (
            <ComponentCard>
              <Label className="text-green-700">Phần trăm giảm giá (%)</Label>
          ...
          {promotion.productApply.length > 0 ? (
            <ProductTable
              products={promotion.productApply.map((item) => ({
                ...item.product,
                promotePricing: item.promotePricing, // Thêm promotePricing vào object sản phẩm
              }))}
